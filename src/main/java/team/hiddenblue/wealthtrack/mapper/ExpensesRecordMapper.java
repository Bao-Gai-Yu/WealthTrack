...
public interface ExpensesRecordMapper {
    @Select("SELECT * FROM expenses_record, user WHERE" +
            " ledger_id = #{ledgerId}" +
            " AND date >= #{startTime}" +
            " AND date <= #{endTime}" +
            " AND expenses_record.user_id = user.user_id")
    public List<ExpensesRecordDto> getPagedByLedgerIdAndTimeZone(RowBounds rowBounds, Integer userId, Integer ledgerId, Date startTime, Date endTime);
    ...
    @Select({"SELECT * FROM expenses_record, user WHERE" +
            ...
            " AND date < #{endTime}" +
            " AND expenses_record.user_id = user.user_id"})
    public List<ExpensesRecordDto> getPagedByLedgerIdAndTimeZoneAndType(RowBounds rowBounds, Integer userId, Integer ledgerId, Boolean type, Date startTime, Date endTime);

    @Select("SELECT * FROM expenses_record, user WHERE" +
            " expenses_record.user_id = #{userId}" +
            " AND ledger_id = #{ledgerId}" +
            " AND date >= #{startTime}" +
            " AND date <= #{endTime}" +
            " AND expenses_record.user_id = user.user_id")
    public List<ExpensesRecordDto> getPagedByUserIdAndTimeZone(RowBounds rowBounds, Integer userId, Integer ledgerId, Date startTime, Date endTime);
    ...
    @Select({"SELECT * FROM expenses_record, user WHERE" +
            ...
            " AND date < #{endTime}" +
            " AND expenses_record.user_id = user.user_id"})
    public List<ExpensesRecordDto> getPagedByUserIdAndTimeZoneAndType(RowBounds rowBounds, Integer userId, Integer ledgerId, Boolean type, Date startTime, Date endTime);
